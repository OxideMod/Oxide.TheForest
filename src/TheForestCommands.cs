...
{
    /// <summary>
    /// Game commands for the core The Forest plugin
    /// </summary>
    public partial class TheForestCore
    {
        ...
        private void VersionCommand(IPlayer player, string command, string[] args)
        {
            if (player.IsServer)
            {
                ...
                $"Branch: {BuildInfo.Current.Scm.Branch}\nOxide.TheForest Version: {TheForestExtension.AssemblyVersion}");*/ // TODO: Implement server version
            }
            else
            {
                string format = Covalence.FormatText(lang.GetMessage("Version", this, player.Id));

                player.Reply(string.Format(format, TheForestExtension.AssemblyVersion, Covalence.GameName, Server.Version, Server.Protocol));
            }
        }
